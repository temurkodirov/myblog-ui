...
export default {
  ...
  methods: {
    ...
    async createPost() {
      ...
      try {
        const result = await axios.post('post/update/' + this.updateId, formData, cfg);
    ...
    async fetchPost() {
      try {
        let result = await axios.get('post/get/' + this.updateId);
        if (result.status === 200) {
          this.post.title = result.data.title;
          this.post.content = result.data.content;
          this.selectedCategory = result.data.categoryId;
        }
      } catch (e)
      {
        console.error(e.message)
      ...
      <div class="col-lg-8 col-xl-6 col-md-12 col-sm-12">
        <div class="card mb-4">
          <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="mb-0">Blogni tahrirlash</h5>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input @change="filesChangeHandler" type="file" id="files" name="files[]"
                         ...
                         multiple
                         >
                <p class="text-muted mt-1">Yangi rasm yuklasangiz eski rasmlar o'chib ketadi <br>
                       Yangi rasm yuklamasez rasmlar o'z xolida qoladi </p>
                </div>
              </div>
              <div class="row mb-3">
              ...
              <div class="row justify-content-end">
                <div class="col-sm-10 ">
                  <div class="d-flex justify-content-end">
                    <button @click="createPost" type="submit" class="btn btn-primary">Yangilash</button>
                  </div>
                </div>
              </div>
            </form>
